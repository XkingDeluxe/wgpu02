...
fn vertex(p:[i8;3], c:[i8;3]) -> Vertex {
    Vertex { position: [p[0] as f32, p[1] as f32, p[2] as f32, 1.0], 
...
impl State{
    async fn new(window: &Window) -> Self {
        ...
        let camera_position = (4.0, 1.5, 4.0).into();
        ...
        let uniform_bind_group_layout = init.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            label: Some("Uniform group layout"),
        });
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        let model_mat = transforms::create_transforms([0.0,0.0,0.0], [dt.sin()*3.0, dt.cos()*3.0, 0.0], [1.0+(dt*50.0).cos()*(dt*50.0).cos()*3.0,1.0,1.0]);
